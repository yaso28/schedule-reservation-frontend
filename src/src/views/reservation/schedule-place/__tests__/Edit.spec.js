...
const record = require('@/testUtils/dummyApiData/schedule-place/get.json')
const id = record.id
const makeFormArgs = {
  name: record.name,
...
describe('Edit', () => {
  ...
  describe('マウント', () => {
    ...
    beforeEach(async () => {
      ...
      wrapper = await pageTester.mount(Edit, { name: 'reservation.schedule-place.edit', params: { id } }, mountOption)
    })
    ...
    test('データ取得処理を呼出', () => {
      expect(apiService.getSchedulePlace).toHaveBeenCalledWith(id)
    })
    ...
    describe('データ取得：成功', () => {
      ...
      describe('URL変更', () => {
        ...
        test('データ取得処理を呼出', () => {
          expect(apiService.getSchedulePlace).toHaveBeenCalledWith(newId)
        })
      ...
      describe('フォーム送信', () => {
        ...
        beforeEach(() => {
          ...
          apiService.updateSchedulePlace = promiseMocker.mock(
            saveResult,
            saveError,
            saveSwitch
          )
        ...
        test('保存処理を呼出', () => {
          expect(apiService.updateSchedulePlace).toHaveBeenCalledWith(id, form)
        })
