...
const records = require('@/testUtils/dummyApiData/reservation-organization/list.json')
...
describe('Reorder', () => {
  ...
  describe('マウント', () => {
    ...
    beforeEach(async () => {
      ...
      wrapper = await pageTester.mount(Reorder, { name: 'reservation.reservation-organization.reorder' }, mountOption)
    })
    ...
    describe('データ取得：成功', () => {
      ...
      describe('フォーム送信', () => {
        ...
        beforeEach(() => {
          ...
          apiService.reorderReservationOrganization = promiseMocker.mock(
            null,
            saveError,
            saveSwitch
          )
        ...
        test('保存処理を呼出', () => {
          expect(apiService.reorderReservationOrganization).toHaveBeenCalledWith(form)
        })
