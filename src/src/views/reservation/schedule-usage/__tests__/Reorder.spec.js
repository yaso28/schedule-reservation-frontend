...
const records = require('@/testUtils/dummyApiData/schedule-usage/list.json')
...
describe('Reorder', () => {
  ...
  describe('マウント', () => {
    ...
    beforeEach(async () => {
      ...
      apiService.getScheduleUsageList = promiseMocker.mock(
        records,
        loadError,
        loadSwtich
      )
      wrapper = await pageTester.mount(Reorder, { name: 'reservation.schedule-usage.reorder' }, mountOption)
    })
    ...
    describe('データ取得：成功', () => {
      ...
      describe('フォーム送信', () => {
        ...
        beforeEach(() => {
          ...
          apiService.reorderScheduleUsage = promiseMocker.mock(
            null,
            saveError,
            saveSwitch
          )
        ...
        test('保存処理を呼出', () => {
          expect(apiService.reorderScheduleUsage).toHaveBeenCalledWith(form)
        })
