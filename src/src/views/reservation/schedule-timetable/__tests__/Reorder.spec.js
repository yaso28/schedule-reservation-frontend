...
const records = require('@/testUtils/dummyApiData/schedule-timetable/list.json')
...
describe('Reorder', () => {
  ...
  describe('マウント', () => {
    ...
    beforeEach(async () => {
      ...
      apiService.getScheduleTimetableList = promiseMocker.mock(
        records,
        loadError,
        loadSwtich
      )
      wrapper = await pageTester.mount(Reorder, { name: 'reservation.schedule-timetable.reorder' }, mountOption)
    })
    ...
    test('データ取得処理を呼出', () => {
      expect(apiService.getScheduleTimetableList).toHaveBeenCalledWith()
    })
    ...
    describe('データ取得：成功', () => {
      ...
      describe('フォーム送信', () => {
        ...
        beforeEach(() => {
          ...
          apiService.reorderScheduleTimetable = promiseMocker.mock(
            null,
            saveError,
            saveSwitch
          )
        ...
        test('保存処理を呼出', () => {
          expect(apiService.reorderScheduleTimetable).toHaveBeenCalledWith(form)
        })
