...
const record = require('@/testUtils/dummyApiData/schedule-timetable/get.json')
const id = record.id
const makeFormArgs = {
  name: record.name,
...
describe('Edit', () => {
  ...
  describe('マウント', () => {
    ...
    beforeEach(async () => {
      ...
      wrapper = await pageTester.mount(Edit, { name: 'reservation.schedule-timetable.edit', params: { id } }, mountOption)
    })
    ...
    test('データ取得処理を呼出', () => {
      expect(apiService.getScheduleTimetable).toHaveBeenCalledWith(id)
    })
    ...
    describe('データ取得：成功', () => {
      ...
      describe('URL変更', () => {
        ...
        test('データ取得処理を呼出', () => {
          expect(apiService.getScheduleTimetable).toHaveBeenCalledWith(newId)
        })
      ...
      describe('フォーム送信', () => {
        ...
        beforeEach(() => {
          ...
          apiService.updateScheduleTimetable = promiseMocker.mock(
            saveResult,
            saveError,
            saveSwitch
          )
        ...
        test('保存処理を呼出', () => {
          expect(apiService.updateScheduleTimetable).toHaveBeenCalledWith(id, form)
        })
